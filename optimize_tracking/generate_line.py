...
def generate_line():
    #####################################################
    # Read general configurations and setup environment #
    #####################################################
    ...
    if not (enable_crabs):
        knob_settings["par_crab1"] = 0.0
        knob_settings["par_crab5"] = 0.0
    ...
    ost.build_sequence(mad, beam=beam_to_configure, configuration=configuration)
    ...
    twiss_dfs, other_data = ost.twiss_and_check(
        mad,
        sequences_to_check,
        tol_beta=tol_beta,
        tol_sep=tol_sep,
    ...
    if len(sequences_to_check) == 2:
        ...
        pm.print_luminosity(
            mad,
            twiss_dfs,
            configuration["nco_IP1"],
            configuration["nco_IP2"],
            configuration["nco_IP5"],
            configuration["nco_IP8"],
        )
    else:
    ...
    if not enable_lumi_control:
        ...
        if mode == "b4_without_bb":
            ...
            if (
                particle_type == "ion"
            ):  # the legacy macro for BB have been checked but not maintained
                raise ValueError
            # Luminosity levelling
            vars_for_legacy_level = ["lumi_ip8", "nco_IP1", "nco_IP2", "nco_IP5", "nco_IP8"]
    ...
    if generate_b4_from_b2:
        ...
        twiss_dfs_b2, other_data_b2 = ost.twiss_and_check(
            mad,
        ...
        twiss_dfs_b4, other_data_b4 = ost.twiss_and_check(
            mad_b4,
            sequences_to_check=["lhcb2"],
            tol_beta=tol_beta,
            tol_sep=tol_sep,
    ...
    if enable_bb_legacy:
        ...
        mad_track.set_variables_from_dict(params=configuration["pars_for_legacy_bb_macros"])
        mad_track.set_variables_from_dict(
            params={f"par_nho_ir{ir}": bbconfig["numberOfHOSlices"] for ir in [1, 2, 5, 8]}
        )
    ...
    if enable_imperfections:
        mad_track.set_variables_from_dict(configuration["pars_for_imperfections"])
    ...
    coupl_corr_info = pm.coupling_correction(
        mad_track,
        n_iterations=configuration["N_iter_coupling"],
        qx_integer=qx_integer,
        qy_integer=qy_integer,
    ...
    # Switch on bb lenses
    mad_track.globals.on_bb_charge = 1.0
    ...
    if sequence_to_track == "lhcb1":
        ...
        mad_track.globals["lagrf400.b2"] = 0.0
    ...
    if enable_bb_legacy:
        ...
        pm.generate_sixtrack_input(
            mad_track,
            seq_name=sequence_to_track,
            bb_df=bb_df_track,
            output_folder="./",
            reference_num_particles_sixtrack=(mad_track.sequence[sequence_to_track].beam.npart),
